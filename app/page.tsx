...
export default function Home() {
    ...
    const loadTimer = window.setTimeout(() => {
      try {
        ...
        if (saved) {
          ...
          if (Array.isArray(parsed.cashflowItems)) setCashflowItems(parsed.cashflowItems.map((item) => ({
            ...item,
            paymentMethod: item.kind === "expense" && item.paymentMethod === "credit" ? "credit" : "debit",
  ...
  const linkedCardExpenses = useMemo(() => cashflowItems.reduce<LinkedCardExpenses>((totals, item) => {
    if (item.kind === "expense" && item.paymentMethod === "credit" && item.creditAccountId) {
      totals[item.creditAccountId] = round((totals[item.creditAccountId] ?? 0) + item.amount);
    }
    return totals;
  }, {}), [cashflowItems]);
  ...
  return <div className="app-shell">
    ...
    <main className="main-area">
      ...
      <div className="page-body">
        ...
        {page === "plan" && <PayoffPlanPage accounts={accounts} plan={plan} extra={extra} availableExtra={availableExtra} strategy={strategy} linkedCardExpenses={linkedCardExpenses} onExtra={setExtra} onStrategy={setStrategy} onAccounts={() => setPage("accounts")} onEditAccount={openEdit}/>}
...
function PayoffPlanPage({ accounts, plan, extra, availableExtra, strategy, linkedCardExpenses, onExtra, onStrategy, onAccounts, onEditAccount }: { accounts: DebtAccount[]; plan: ReturnType<typeof calculatePlan>; extra: number; availableExtra: number; strategy: PayoffStrategy; linkedCardExpenses: LinkedCardExpenses; onExtra: (value: number) => void; onStrategy: (strategy: PayoffStrategy) => void; onAccounts: () => void; onEditAccount: (account: DebtAccount) => void }) {
