...
export function transactionAdjustedAccounts(openingAccounts: DebtAccount[], transactions: LedgerTransaction[]) {
...
export function buildProgressBalanceView(openingAccounts: DebtAccount[], transactions: LedgerTransaction[], snapshots: PayoffSnapshot[]) {
  const activeTransactions = transactions.filter((transaction) => !transaction.deletedAt);
  const currentAccounts = transactionAdjustedAccounts(openingAccounts, activeTransactions);
