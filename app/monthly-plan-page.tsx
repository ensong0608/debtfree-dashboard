...
export default function MonthlyPlanPage(props: Props) {
  ...
  return <div className="screen monthly-plan-screen">
    ...
    {!hasMonth ? <section className="month-start-card"><span>New month</span><h2>Set up {monthLabel(month)}</h2><p>Copy recurring items from last month or start clean. One-time adjustments are never copied.</p><div>{previousHasItems && <button className="primary" type="button" onClick={props.onCopyPrevious}>Copy recurring items</button>}<button className="secondary" type="button" onClick={props.onStartBlank}>Start with no entries</button></div></section> : <>
      ...
      <section className="debt-capacity-card" aria-labelledby="debt-capacity-title"><div><span>Supports your payoff plan</span><h2 id="debt-capacity-title">Available debt payment</h2><p>Take-home income minus essential planned spending minus the safety buffer.</p></div><div className="capacity-fields"><label><span>Monthly safety buffer</span><div><b>$</b><input aria-label="Monthly safety buffer" type="number" min="0" step="0.01" inputMode="decimal" value={settings.safetyBuffer || ""} placeholder="0.00" onChange={(event) => props.onSettings({ ...settings, safetyBuffer: Math.max(0, Number(event.target.value) || 0) })}/></div></label><label><span>Monthly debt-payment target</span><div><b>$</b><input aria-label="Monthly debt-payment target" type="number" min="0" step="0.01" inputMode="decimal" value={settings.debtPaymentTarget || ""} placeholder="0.00" onChange={(event) => props.onSettings({ ...settings, debtPaymentTarget: Math.max(0, Number(event.target.value) || 0) })}/></div></label></div><dl><div><dt>Take-home income</dt><dd>{currency.format(totals.plannedIncome)}</dd></div><div><dt>Essential planned spending</dt><dd>-{currency.format(totals.plannedSpending)}</dd></div><div><dt>Safety buffer</dt><dd>-{currency.format(totals.safetyBuffer)}</dd></div><div className="available"><dt>Available debt payment</dt><dd>{currency.format(totals.availableDebtPayment)}</dd></div></dl>{settings.debtPaymentTarget > totals.availableDebtPayment && <p className="form-error" role="alert">The debt-payment target is {currency.format(settings.debtPaymentTarget - totals.availableDebtPayment)} above the amount currently available.</p>}</section>
