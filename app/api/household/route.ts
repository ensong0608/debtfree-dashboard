...
import { dashboardDataErrorMessage, parseHouseholdWriteJson, serializeDashboardBackup } from "@/app/dashboard-data";
...
export async function PUT(request: Request) {
  ...
  let contract;
  try { contract = parseHouseholdWriteJson(raw); }
  catch (error) {
    return Response.json({ error: dashboardDataErrorMessage(error) }, { status: 400 });
  }
  const now = Date.now();
  await context.db.prepare("UPDATE household_state SET payload = ?, revision = revision + 1, updated_by = ?, updated_at = ? WHERE household_id = ?")
    .bind(serializeDashboardBackup(contract), context.user.email, now, context.member.household_id).run();
