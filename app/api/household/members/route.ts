import { householdContext, listMembers, normalizeEmail } from "../store";
...
export async function POST(request: Request) {
  ...
  if (context.member.role !== "owner") return Response.json({ error: "Only the household owner can add admins" }, { status: 403 });
  ...
  const now = Date.now();
  if (existing) {
    await context.db.prepare("UPDATE household_members SET role = 'admin', updated_at = ? WHERE email = ?").bind(now, email).run();
  } else {
    await context.db.prepare("INSERT INTO household_members (email, household_id, display_name, role, status, invited_by, created_at, updated_at) VALUES (?, ?, NULL, 'admin', 'invited', ?, ?, ?)")
      .bind(email, context.member.household_id, context.user.email, now, now).run();
  }
...
export async function DELETE(request: Request) {
  const context = await householdContext();
  if (!context) return Response.json({ error: "Sign in required" }, { status: 401 });
  if (context.member.role !== "owner") return Response.json({ error: "Only the household owner can remove admins" }, { status: 403 });
